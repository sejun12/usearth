...
// API로부터 데이터 가져오기
fetch("/lists/api/freeboard/category")
    ...
    .then(data => {
        ...
        data.forEach(category => {
            ...
            button.textContent = category.postCategoryName + "게시판";
...
async function getPosts(page) {
    ...
    const url = `/lists/api/freeboard/1?page=${page}`; // 실제 서버 API 엔드포인트를 사용해야 합니다.
...
function appendPost(freeboard) {
    const li = document.createElement('li');
    li.innerHTML = `
         <div class="maintenanceFeeList" data-post-id="${freeboard.id}">
            <div style="width: 120px;" class="barBtnOne">${freeboard.postCategory}게시판</div>
            <div style="width: 120px;" class="barBtnOne">${freeboard.userName}</div>
            <div style="width: 250px; text-align: left;" class="barBtnOne">${freeboard.postTitle}</div>
            <div style="width: 250px;" class="barBtnOne">${freeboard.postWriteDate}</div>
            <div class="barBtnOne">
    ...
    // 호버(hover) 스타일 추가
    li.style.transition = "background-color 0.3s, border-radius 0.3s";
    li.style.borderRadius = "4px";
    li.style.backgroundColor = "transparent";

    li.addEventListener("mouseover", function() {
        li.style.backgroundColor = "rgba(162, 218, 255, 0.35)";
    });
    ...
    li.addEventListener('click', function() {
        // 해당 li 엘리먼트의 data-post-id 속성 값을 가져옴
        const postId = approvalBtn.closest('.maintenanceFeeList').getAttribute('data-post-id');

        // postId를 사용하여 상세보기 페이지로 이동
        window.location.href = `/admin/board-detail/${postId}`;
    });
    ...
    approvalBtn.addEventListener('click', function (event) {
        ...
        if (handled) {
            if (isHandledOnRight) {
                ...
                // postId를 사용하여 삭제 요청을 보냄
                sendDeleteRequest(postId);
                // 해당 엘리먼트를 삭제
                // approvalBtn.closest('.maintenanceFeeList').remove();
            }
...
// 서버로 DELETE 요청을 보내는 함수
async function sendDeleteRequest(postId) {
    // DELETE 요청을 보내는 URL 설정
    const url = `/lists/api/freeboard/delete/${postId}`; // 적절한 URL로 변경해야 합니다.
...
async function fetchData(page) {
    ...
    const baseUrl = `/lists/api/freeboard/result/${id}`;
    ...
    if (searchCategory) {
        const category = searchCategory.replace("게시판", "");
        params.push(`postCategory=${category}`);
    }
    if (searchTitle) {
        params.push(`postTitle=${searchTitle}`);
    }
